...
/**
 * Holonomic LQR path follower.
 *
 * MODEL CHOICE, and why it differs from a textbook LQR example:
 *
 * A holonomic drivetrain closed around wheel velocity behaves, to first order,
 * like a VELOCITY SOURCE. You command a velocity and it produces roughly that
 * velocity. So the model here is:
 *
 *     state   x = [x, y, theta]        (field frame)
 *     control u = [vx, vy, omega]      (field frame)
 *     x[k+1] = x[k] + dt * u[k]   ->   A = I,  B = dt*I
 *
 * The tempting alternative is a double integrator with acceleration as the
 * control. That is worse here, because the code would then have to integrate
 * the LQR output into a velocity command before sending it to a drivetrain
 * that is ALREADY a velocity source. That inserts an integration the model does
 * not know about, and the mismatch shows up as overshoot you cannot tune away
 * without re-deriving the model. Matching the model to the actual signal path
 * is the fix.
 *
 * A pleasant consequence: with A=I and B=dt*I and diagonal Q,R, the Riccati
 * equation decouples into three independent scalar equations, each solvable in
 * closed form. The exact gain is
 *
 *     p = [q*dt^2 + sqrt(q^2*dt^4 + 4*dt^2*q*r)] / (2*dt^2)
 *     k = dt*p / (r + p*dt^2)
 *
 * and k tends to sqrt(q/r) as dt tends to zero. Do not treat sqrt(q/r) as the
 * answer at realistic loop rates: at dt=0.02 it overstates the true gain by
 * about 3%, at dt=0.005 by 0.8%, at dt=0.001 by 0.16%. Use it as a mental
 * estimate for tuning, not as a value to hardcode.
 *
 * Either way the gains are physically interpretable: k = 3.06 means "command
 * 3.06 in/s of correction per inch of position error", and the closed-loop
 * time constant is roughly 1/k seconds. That interpretability is the practical
 * reason to prefer this model over a double integrator, where the gains are
 * just numbers.
 *
 * FEEDFORWARD + FEEDBACK: the command sent to the drivetrain is
 *     v_command = v_reference (feedforward from the profile)
 *               + K * (reference_pose - measured_pose)   (feedback)
 * Feedforward does the bulk of the work; feedback only cleans up error. A
 * follower with feedback alone always lags, because it needs error to exist
 * before it produces output.
 *
 * This class contains NO Android or FTC SDK imports on purpose, so it compiles
 * and unit-tests on a desktop JVM and can be shared with the visualizer.
 */

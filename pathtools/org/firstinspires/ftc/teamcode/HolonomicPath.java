...
/**
 * Geometry layer for a holonomic path follower.
 *
 * COORDINATE CONVENTION (everything in this file assumes it):
 *   - Units are inches, angles are radians.
 *   - Origin at field center. +x right, +y up (standard math axes).
 *   - Heading is CCW-positive, measured from +x.
 * Convention mismatches are the single most common source of "the robot drove
 * into the wall" bugs, so pick one and keep it everywhere.
 *
 * A path here is PURE GEOMETRY. It knows shape, not speed. Timing comes from
 * the motion profile in LQRPathFollower. Keeping those separate means you can
 * re-time a path without touching its shape, and visualize shape without
 * needing a profile.
 *
 * The parameter t always runs 0..1 and is NOT proportional to distance. On a
 * bezier, t = 0.5 is usually not the halfway point. That is what ArcLengthTable
 * exists to correct.
 */
